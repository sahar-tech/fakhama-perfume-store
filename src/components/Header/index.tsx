...
const Header = () => {
  ...
  return (
    ...
    >
      <div className="max-w-[1170px] mx-auto px-6 sm:px-7.5 xl:px-0">
        {/* <!-- header top start --> */}
        <div
          className={`flex flex-col lg:flex-row gap-5 items-end lg:items-center xl:justify-between ease-out duration-200 ${stickyMenu ? "py-1" : "py-2"
            }`}>
      ...
      <SearchWithCategories onSearch={handleSearch} />
                  ...
                  {menuData.map((menuItem, i) =>
                    menuItem.submenu ? (
                      ...
                      >
                        <Link
                          href={menuItem.path}
                          className={`hover:text-blue text-custom-sm font-medium text-foreground flex ${stickyMenu ? "xl:py-4" : "xl:py-6"
                            }`}
                        >
                          {menuItem.title}
                        </Link>
